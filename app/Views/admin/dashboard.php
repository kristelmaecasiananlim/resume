...
<body>
    ...
    <div class="dashboard-header">
        <div class="section-content">
            ...
            <div class="dashboard-controls">
                <a href="<?= base_url('/') ?>">← Back to Portfolio</a>
                ...
                <table class="table">
                    ...
                    <?php foreach($contacts as $contact): ?>
                        <tr>
                            ...
                            <td><a href="mailto:<?= htmlspecialchars($contact['email']) ?>" style="color: #2e7d32;"><?= htmlspecialchars($contact['email']) ?></a></td>
                            <td><?= htmlspecialchars($contact['subject']) ?></td>
                            <td style="max-width: 300px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;"><?= htmlspecialchars($contact['message']) ?></td>
                            <td><?= date('M d, Y', strtotime($contact['created_at'])) ?></td>
                            <td>
                                <a href="javascript:void(0)" class="action-link action-edit" onclick="editContact(<?= $contact['id'] ?>, '<?= htmlspecialchars(addslashes($contact['name']), ENT_QUOTES) ?>', '<?= htmlspecialchars(addslashes($contact['email']), ENT_QUOTES) ?>', '<?= htmlspecialchars(addslashes($contact['subject']), ENT_QUOTES) ?>', '<?= htmlspecialchars(addslashes($contact['message']), ENT_QUOTES) ?>');">Edit</a>
                                <a href="<?= base_url('admin/deleteContact/' . $contact['id']) ?>" class="action-link action-delete" onclick="return confirm('Are you sure you want to delete this message?');">Delete</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                ...
                <table class="table">
                    ...
                    <?php foreach($projects as $project): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars($project['date_range']) ?></td>
                            <td>
                                <span style="padding: 0.4rem 0.8rem; border-radius: 4px; font-size: 0.85rem; font-weight: 600; background: <?= $project['status'] === 'active' ? '#c8e6c9' : '#ffcccc' ?>; color: <?= $project['status'] === 'active' ? '#2e7d32' : '#c62828' ?>;">
                                    <?= ucfirst($project['status']) ?>
                                </span>
                            </td>
                            <td>
                                <a href="javascript:void(0)" class="action-link action-edit" onclick="editProject(<?= $project['id'] ?>, '<?= htmlspecialchars(addslashes($project['title']), ENT_QUOTES) ?>', '<?= htmlspecialchars(addslashes($project['description']), ENT_QUOTES) ?>', '<?= htmlspecialchars(addslashes($project['date_range']), ENT_QUOTES) ?>', '<?= htmlspecialchars(addslashes($project['logo_path'] ?? ''), ENT_QUOTES) ?>', '<?= htmlspecialchars(addslashes($project['project_url'] ?? ''), ENT_QUOTES) ?>', '<?= htmlspecialchars(addslashes($project['technologies'] ?? ''), ENT_QUOTES) ?>', '<?= $project['status'] ?>', <?= $project['display_order'] ?>);">Edit</a>
                                <a href="<?= base_url('admin/duplicateProject/' . $project['id']) ?>" class="action-link" style="color: #7b68ee;" title="Duplicate Project">Duplicate</a>
                                <a href="<?= base_url('admin/deleteProject/' . $project['id']) ?>" class="action-link action-delete" onclick="return confirm('Are you sure you want to delete this project?');">Delete</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
    ...
    <script>
        function editContact(id, name, email, subject, message) {
            ...
            document.getElementById('editForm').action = '<?= base_url('admin/updateContact/') ?>' + id;
        ...
        function openCreateProjectModal() {
            ...
            document.getElementById('projectForm').action = '<?= base_url('admin/createProject') ?>';
        ...
        function editProject(id, title, description, dateRange, logoPath, url, technologies, status, order) {
            ...
            document.getElementById('projectForm').action = '<?= base_url('admin/updateProject/') ?>' + id;
            document.getElementById('projectModal').classList.add('show');
        }
